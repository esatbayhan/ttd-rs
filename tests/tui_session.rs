...
fn quick_entry_hidden_by_current_filter_preserves_previous_selection() {
    ...
    session.dispatch_key("j").unwrap();
    let _expected_file_name = session.selected_task().unwrap().id.file_name().to_string();
    let _expected_description = session.selected_task().unwrap().task.description.clone();
